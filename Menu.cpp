...
Menu::Menu(){
    ...
    for(int i=0;i<Num_TowerType; i++)
    {
        ...
        //std::cout << TowerClass[i] << std::endl; //H?
...
void Menu::Draw()
{
    ...
    for(int i=0; i < Num_TowerType; i++)
    {
        ...
        //std::cout << "ok" << std::endl; //only five
        if(!Enough_Coin(i)) al_draw_filled_rectangle(pos_x, pos_y, pos_x + ThumbWidth, pos_y + ThumbHeight, al_map_rgba(100, 100, 100, 100));
        else if(i == selectedTower) al_draw_rectangle(pos_x, pos_y, pos_x + ThumbWidth, pos_y + ThumbHeight, al_map_rgb(255, 0, 0), 0);
        else al_draw_rectangle(pos_x, pos_y, pos_x + ThumbWidth, pos_y + ThumbHeight, al_map_rgb(255, 255, 255), 0);
    }
