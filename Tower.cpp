...
bool Tower::DetectAttack(Monster *monster)
{
    ...
    if(Circle::isOverlap(this->circle, monster->getCircle()))
    {
        ...
        *
        */
        /*if(monster->getHealth()>0) attack_counter = 1;
        else attack_counter = 0;*/

        if(attack_counter==0){
            attack = new Attack(this->circle, monster->getCircle(), this->attack_harm_point, this->attack_velocity, this->attack_img);
            this->attack_set.push_back(attack);
            willAttack = (!TriggerAttack(monster));
        }
    }
...
bool Tower::TriggerAttack(Monster *monster) //true = monster is dead; false monster alive
{
    ...
    for(unsigned int i = 0; i < this->attack_set.size(); i++)
    {
        if(Circle::isOverlap(attack_set[i]->getCircle(), monster->getCircle())) //the attack circle overlaps the monster
        {
            if(monster->Subtract_HP(attack_harm_point)){ //if monster's hp reduced to 0
                return true;
            }
            /*TODO:*/
            /*1. Reduce the monster HP by the harm point*/ //ok
            /*2. Erase and delete the attack from attack set*/
            /*3. Return true if the monster's HP is reduced to zero*/ //ok
        }
    }
    return false;
}
...
{
    //std::cout << "Tower::UpdateAttack" << std::endl;
    for(unsigned int i=0; i < this->attack_set.size(); i++)
    {
